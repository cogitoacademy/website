...
import type { CalendarCompetition, CompetitionColor } from "@/components/competition-calendar";
import {
  DefaultEndHour,
  DefaultStartHour,
  EndHour,
  StartHour,
} from "@/components/competition-calendar/constants";
...
interface EventDialogProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (event: CalendarCompetition) => void;
...
export function EventDialog({ event, isOpen, onClose, onSave, onDelete }: EventDialogProps) {
  ...
  const [color, setColor] = useState<CompetitionColor>("sky");
  ...
  useEffect(() => {
    if (event) {
      ...
      setColor((event.color as CompetitionColor) || "sky");
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="grid gap-4 py-4">
          ...
          <fieldset className="space-y-4">
            <legend className="font-medium text-foreground text-sm leading-none">Etiquette</legend>
            <RadioGroup
              className="flex gap-1.5"
              defaultValue={colorOptions[0]?.value}
              onValueChange={(value) => setColor(value as CompetitionColor)}
              value={color}
            >
