...
export function OrganizationJsonLd({ locale = "id" }: OrganizationJsonLdProps) {
	const organization = {
		...
		sameAs: [
			"https://www.instagram.com/cogitoacademy.id/",
			"https://wa.me/62881011990195",
		],
		areaServed: {
			"@type": "Country",
			name: "Indonesia",
...
export function CourseJsonLd({ locale = "id" }: CourseJsonLdProps) {
	const courses = [
		...
		},
		{
			"@type": "Course",
			name:
				locale === "id" ? "Kelas Ekstrakurikuler" : "Extracurricular Classes",
			description:
				locale === "id"
		...
		mainEntity: [
			{
				...
				name:
					locale === "id"
						? "Apa itu Cogito Academy?"
						: "What is Cogito Academy?",
				acceptedAnswer: {
					"@type": "Answer",
					text:
						locale === "id"
			...
			},
			{
				...
				name:
					locale === "id"
						? "Program apa saja yang ditawarkan?"
