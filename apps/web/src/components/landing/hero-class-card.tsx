...
function getModalContent(tutorType?: string): ModalContent {
  if (tutorType === "Tutor Kelas Intensif") {
    return {
      ...
      ctaText: "Konsultasi Gratis sekarang dengan Tim Cogito",
  ...
  return (
    <div>
      <div className="space-y-3">
        <h3 className="text-neutral-1000">
          Saat ini telah tersedia untuk semua jenis perlombaan.
        </h3>
...
export function ClassCard({
  ...
  const [activeRegulerTab, setActiveRegulerTab] = useState(
    REGULER_SUBJECTS[0].slug,
  );
  const locale = useLocale();
  const isId = locale === "id";
  const modalContent = getModalContent(tutorType);
