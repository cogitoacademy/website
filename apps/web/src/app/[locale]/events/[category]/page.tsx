...
export default async function EventCategoryPage({ params }: Props) {
  ...
  let events: Event[] = [];
  try {
    const sanityEvents = await client.fetch<SanityEvent[]>(EVENTS_BY_CATEGORY_QUERY, {
  ...
  return (
    <Card className="overflow-hidden border-neutral-200 shadow-sm transition-shadow hover:shadow-md">
      ...
      <CardHeader>
        ...
        <CardTitle className="line-clamp-2 text-lg md:text-xl">{event.title}</CardTitle>
      </CardHeader>
      ...
      <CardContent className="space-y-3">
        ...
        {event.summary && (
          <p className="line-clamp-4 text-neutral-600 text-sm leading-relaxed">{event.summary}</p>
        )}
      </CardContent>
