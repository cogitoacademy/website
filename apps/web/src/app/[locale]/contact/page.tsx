  ...
  return (
    <main className="relative z-1 flex flex-col">
      ...
      <section className="mx-4 w-auto max-w-[calc(80rem-32px)] space-y-4 rounded-4xl bg-primary-100 p-6 pb-60! sm:p-8 md:p-12 lg:space-y-7.5 lg:p-15 xl:mx-auto xl:w-full">
        ...
        <div className="flex flex-row flex-wrap items-center justify-center gap-2 md:gap-4 lg:gap-6">
          {BUTTON_CONTENT[locale as Locale].map((button) => (
            <a key={button.href as string} href={button.href as string} className="w-auto">
              <Button className="w-auto cursor-pointer" variant="primary" size="md">
                {button.icon}
                {button.label}
              </Button>
            </a>
